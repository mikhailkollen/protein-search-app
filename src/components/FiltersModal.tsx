...
import { setFilters, setIsFiltersModalOpen } from "../features/search/searchSlice"
...
const FiltersModal = () => {
  ...
  const [lengthFilter, setLengthFilter] = useState<any>({ min: null, max: null });

  const [url, setUrl] = useState<any>(`https://rest.uniprot.org/uniprotkb/search?facets=model_organism,proteins_with,annotation_score&query=(${searchQuery || "*"})${appliedFilters ? (appliedFilters as any)?.map((filter: any) => {
  ...
  useEffect(() => {
    ...
    const filters = (appliedFilters as any)?.map((filter: any) => {
      return `%20AND%20(${filter.name}:${filter.value})`
    }
    ).join('')

    setUrl(`https://rest.uniprot.org/uniprotkb/search?facets=model_organism,proteins_with,annotation_score&query=(${searchQuery || "*"})${appliedFilters && filters}`)
  ...
  const getDynamicFilters = async () => {
    ...
    const minLengthResponse = await fetch(`https://rest.uniprot.org/uniprotkb/search?query=${searchQuery || "*"
      }&format=json&fields=accession,id,gene_names,organism_name,length,cc_subcellular_location&size=1&sort=length%20asc`)
    const maxLengthResponse = await fetch(`https://rest.uniprot.org/uniprotkb/search?query=${searchQuery || "*"
      }&format=json&fields=accession,id,gene_names,organism_name,length,cc_subcellular_location&size=1&sort=length%20desc`);
  ...
  const handleFilterChange = (event: any) => {
    ...
    setSelectedFilters((prevState: any) => {
      const filterExists = prevState.some((item: any) => (
  ...
  useEffect(() => {
    getDynamicFilters().then((data) => {
      ...
      data.forEach((filter: any) => {
        const selectElement = document.getElementById(filter.name) as HTMLSelectElement;
  ...
  return (
    <Wrapper>
      ...
      <form className="filters-form" onSubmit={(e) => handleSubmit(e)}>
        ...
        <input type="text" name="gene" id="gene" placeholder="Enter gene name" onChange={(e) => handleFilterChange(e)} />
        {dynamicFilters.map((filter: any) => {
          return (
            <div className="filter-container" key={filter.name}>
              <label htmlFor={filter.name}>{filter.label}</label>
              <select name={filter.name} id={filter.name} onChange={(e) => handleFilterChange(e)}>
                {filter.values.map((value: any) => {                 
                  
                  return (
                    <option value={value.value} key={`${value.label}${value.count}`}>
                      {value.label ? value.label : value.value} ({value.count})
                    </option>
                  )
                })}
              </select>
        ...
        <div className="length-filter-container">
          ...
          <div className="length-input">
            <input type="number" id="length" name="minLength" min={lengthFilter.min && lengthFilter.min} defaultValue={lengthFilter.min && lengthFilter.min} onChange={(e) => handleFilterChange(e)} />
            <hr />
            <input type="number" name="maxLength" id="length" max={lengthFilter.max && lengthFilter.max} defaultValue={lengthFilter.max && lengthFilter.max} onChange={(e) => handleFilterChange(e)} />
          </div>
        </div>
        <div className="filters-btn-container">
          <button type="button" onClick={()=>dispatch(setIsFiltersModalOpen(false))}>{"Cancel"}</button>
